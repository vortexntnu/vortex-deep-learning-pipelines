...
## Run
```bash
ros2 launch vortex_yolo_classifiy classifier_node.launch.py
```
You can also select the compute device with the `device` argument:
```bash
# Run inference on CPU
ros2 launch vortex_yolo_classifiy classifier_node.launch.py device:=cpu

# Run inference on GPU 0
ros2 launch vortex_yolo_classifiy classifier_node.launch.py device:=0
```
